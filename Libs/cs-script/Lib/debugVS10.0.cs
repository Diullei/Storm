...
namespace VS100
{
    class Script
    {
        ...
        static List<string> CreateSearchDirs()
        {
            ...
            AddSearchDir(Environment.ExpandEnvironmentVariables(@"%CSSCRIPT_DIR%\lib"));

            Settings settings = GetSystemWideSettings();
            if (settings != null)
            {
                foreach (string dir in settings.SearchDirs.Split(';'))
                    if (dir != "" && !searchDirsList.Contains(dir))
                        retval.Add(Path.GetFullPath(Environment.ExpandEnvironmentVariables(dir)));
            }
        ...
        static string[] SearchDirs
        {
            get
            {
                if (searchDirsList == null)
                {
                    searchDirsList = new List<string>();

                    if (scriptFile != null)
                        searchDirsList.Add(Path.GetDirectoryName(Path.GetFullPath(scriptFile)));

                    searchDirsList.Add(Environment.ExpandEnvironmentVariables(@"%CSSCRIPT_DIR%\lib"));
                    ...
                    if (settings != null)
                    {
                        foreach (string dir in settings.SearchDirs.Split(';'))
                            if (dir != "" && !searchDirsList.Contains(dir))
                                searchDirsList.Add(Path.GetFullPath(Environment.ExpandEnvironmentVariables(dir)));
                    }
